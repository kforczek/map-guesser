...
constexpr short int START_MAP_HEIGHT = 300;

constexpr short int MAP_X_INCR = 150;
constexpr short int MAP_Y_INCR = 100;
}
...
void StreetViewPage::resizeAndMoveMap(int newWidth /*= 0*/, int newHeight /*= 0*/)
{
    ...
    {
        newWidth = static_cast<int>(m_mapWidthPercent * width());
        newHeight = static_cast<int>(m_mapHeightPercent * height());
    }
...
void StreetViewPage::updateMapRatios()
{
    ...
    double mapHeight = m_interactiveMap.height() != 0 ? m_interactiveMap.height() : START_MAP_HEIGHT;

    m_mapWidthPercent = mapWidth / width();
    m_mapHeightPercent = mapHeight / height();
}
...
{
    if (m_interactiveMap.width() <= 150 || m_interactiveMap.height() <= 100)
        return;
