...
#include "ui/widgets/leaderboard_info_box.h"
...
constexpr QSize INFO_LABEL_SIZE{ 220, 40 };
...
constexpr QPoint infoLabelPos(int row)
{
    constexpr int marginX = 0;
    constexpr int marginY = 10;
    constexpr int labelHeight = 40;

    return QPoint{marginX, marginY + (labelHeight * row)};
}

// QSize rightInfoBoxSize(size_t playersCnt)
// {
//     constexpr int width = 220;
//     const int height = 60 + (playersCnt * 20);
...
void GameplayPage::setupMembers()
{
    ...
    m_rightInfoBox = new widgets::LeaderboardInfoBox(this);
...
void GameplayPage::setupGeometries()
{
    ...
    m_rightInfoBox->move(width() - m_rightInfoBox->width() - 10, 10);

    m_roundInfoLabel->setFixedSize(INFO_LABEL_SIZE);
    m_roundInfoLabel->move(infoLabelPos(0));
    m_roundInfoLabel->setAlignment(Qt::AlignCenter);
    m_roundInfoLabel->setText(formatRoundInfo(m_currRoundNumber, m_roundsCnt));

    m_timerInfoLabel->setFixedSize(INFO_LABEL_SIZE);
    m_timerInfoLabel->move(infoLabelPos(1));
...
void GameplayPage::prepareLeftInfoBox()
{
    if (m_roundTimeLimit > 0)
    {
        m_timerInfoLabel->setVisible(true);
...
void GameplayPage::moveRightInfoBox()
{
    if (!m_initialized)
        return;

    constexpr int margin = 10;
    m_rightInfoBox->move(width() - m_rightInfoBox->width() - margin, margin);
}
